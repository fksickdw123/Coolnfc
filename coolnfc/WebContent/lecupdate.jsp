...
<table border="1">
<tr bgcolor="#99CCFF" ><th>강의실 </th><th>passwd</th><th>passwd_close</th><th>삭제</th></tr>
<%
		for(Memo memo : (ArrayList<Memo>)Bean.getLecroom()){
	%>
<tr>
	<td><%=memo.getLecroom() %></td>
	<td><%=memo.getDoor_pw() %></td>
	<td><%=memo.getDoor_pwclose() %></td>
	
	<td><a href="control.jsp?action=deletelec&lecroom=<%=memo.getLecroom()%>">X</a></td>
